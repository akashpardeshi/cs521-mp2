        ...
        for m in nl.affine_range(M // TILE_M):
            ...
            for n in nl.affine_range(N // (num_rows * TILE_N)):
                ...
                for k in nl.affine_range(K // TILE_K):
                    ...
                    X_tile = nl.ndarray((TILE_K, filter_height + (num_rows - 1), input_width), dtype=X.dtype, buffer=nl.sbuf)
                    W_tile[...] = nl.load(W[m * TILE_M:(m + 1) * TILE_M, k * TILE_K:(k + 1) * TILE_K, :, :])
                    X_tile[...] = nl.load(X[b, k * TILE_K:(k + 1) * TILE_K, n:n + filter_height + (num_rows - 1), :])
                    ...
                    for i in nl.affine_range(filter_height):
                        for j in nl.affine_range(filter_width):
                            ...
                            if num_rows == 2:
                                X_block = X_tile[:, i + 1, j:j + out_width]
                                res_psum += nl.matmul(W_tile[:, :, i, j], X_block)
